...
public class Graph {
  ...
  public static void graph(String expression) throws FileNotFoundException {
    ...
    for (x = 0; x < height; x++) {
      img.setRGB(x, y, color); // for all x values draw a line at the middle of the image
      img.setRGB(y, x, color); // Rotated it 90 degrees. This is just a more convinet way to draw it
    }
    ...
    for(x = 0; x < width; x++) {
      ...
      tempx = ((double)x) / 100 - 10; // convert standard double x to pixel value
      ...
      if (y < 2000 && y > 0) { // if the y is within the range of the graph
        img.setRGB(x, y, color); // sets the value to color red
      }
    }
